...
impl<'a, 'tcx> MirBorrowckCtxt<'a, 'tcx> {
    ...
    fn show_mutating_upvar(
        ...
        let local_def_id = id.expect_local();
        let tables = tcx.typeck(local_def_id);
        let hir_id = tcx.hir().local_def_id_to_hir_id(local_def_id);
        let (span, closure_kind_origin) = &tables.closure_kind_origins()[hir_id];
        let reason = if let PlaceBase::Upvar(upvar_id) = closure_kind_origin.base {
            ...
            for captured_place in captured_places {
                let captured_place_kinds = captured_place
                    ...
                    .map(|proj| proj.kind)
                    .collect::<Vec<_>>();
                let place_kinds = closure_kind_origin
                    .projections
                    .iter()
                    .map(|proj| proj.kind)
                    .collect::<Vec<_>>();
                if rustc_middle::hir::place::is_ancestor_or_same_capture(
